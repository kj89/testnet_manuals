...
for (( ;; )); do
	...
	defundd tx distribution withdraw-rewards $DEFUND_VALOPER_ADDRESS --from=$WALLET --commission --chain-id=$DEFUND_CHAIN_ID --fees=250ufetf --yes
	...
	echo -e "Your total balance: \033[0;32m$AMOUNT_STRING\033[0m"
	 defundd tx staking delegate $DEFUND_VALOPER_ADDRESS $AMOUNT_STRING --from $WALLET --chain-id $DEFUND_CHAIN_ID --fees=250ufetf --yes
