...
for (( ;; )); do
	...
	seid tx distribution withdraw-rewards $SEI_VALOPER_ADDRESS --from=$WALLET --commission --chain-id=$SEI_CHAIN_ID --yes
	...
	seid tx staking delegate $SEI_VALOPER_ADDRESS $AMOUNT_STRING --from $WALLET --chain-id $SEI_CHAIN_ID --yes
