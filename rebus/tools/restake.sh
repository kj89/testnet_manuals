...
for (( ;; )); do
	...
	rebusd tx distribution withdraw-rewards $REBUS_VALOPER_ADDRESS --from=$WALLET --commission --chain-id=$REBUS_CHAIN_ID --fees=250arebus --yes
	...
	echo -e "Your total balance: \033[0;32m$AMOUNT_STRING\033[0m"
	 rebusd tx staking delegate $REBUS_VALOPER_ADDRESS $AMOUNT_STRING --from $WALLET --chain-id $REBUS_CHAIN_ID --fees=250arebus --yes
