...
for (( ;; )); do
	...
	echo -e "${PASSWORD}\n" | strided tx distribution withdraw-rewards $STRIDE_VALOPER_ADDRESS --from=$WALLET --commission --chain-id=$STRIDE_CHAIN_ID --fees=250ukuji --yes
	...
	echo -e "${PASSWORD}\n" |  strided tx staking delegate $STRIDE_VALOPER_ADDRESS $AMOUNT_STRING --from $WALLET --chain-id $STRIDE_CHAIN_ID --fees=250ukuji --yes
