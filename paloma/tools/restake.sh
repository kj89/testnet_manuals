...
for (( ;; )); do
	...
	echo palomad tx distribution withdraw-rewards $PALOMA_VALOPER_ADDRESS --from=$WALLET --commission --chain-id=$PALOMA_CHAIN_ID --yes
	...
	echo palomad tx staking delegate $PALOMA_VALOPER_ADDRESS $AMOUNT_STRING --from $WALLET --chain-id $PALOMA_CHAIN_ID --yes
