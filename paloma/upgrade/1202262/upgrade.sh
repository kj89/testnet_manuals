...
for((;;)); do
	...
	if ((height>=$BLOCK)); then
		...
		wget -O - https://github.com/palomachain/paloma/releases/download/${VERSION}/paloma_Linux_x86_64.tar.gz | \
		sudo tar -C /usr/local/bin -xvzf - palomad
		sudo chmod +x /usr/local/bin/palomad
		wget -O - https://github.com/palomachain/pigeon/releases/download/${VERSION}/pigeon_Linux_x86_64.tar.gz | \
