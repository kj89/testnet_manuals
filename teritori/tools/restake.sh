...
for (( ;; )); do
	...
	echo -e "${PASSWORD}\n" | teritorid tx distribution withdraw-rewards $TERITORI_VALOPER_ADDRESS --from=$WALLET --commission --chain-id=$TERITORI_CHAIN_ID --fees=250ueteritori --yes
	...
	AMOUNT=$(($AMOUNT - 500))
	AMOUNT_STRING=$AMOUNT"ueteritori"
	echo -e "Your total balance: \033[0;32m$AMOUNT_STRING\033[0m"
	echo -e "${PASSWORD}\n" |  teritorid tx staking delegate $TERITORI_VALOPER_ADDRESS $AMOUNT_STRING --from $WALLET --chain-id $TERITORI_CHAIN_ID --fees=250ueteritori --yes
