...
# Set up monitoring and alerting for osmosis validator
...
| KEY |VALUE |
|---------------|-------------|
| **bond_denom** | Denominated token name, for example, `uosmo` for osmosis testnet. You can find it in genesis file |
| **bench_prefix** | Prefix for chain addresses, for example, `osmosis` for osmosis testnet. You can find it in public addresses like this **osmosis**_valoper1zyyz4m9ytdf60fn9yaafx7uy7h463n7alv2ete_ |
| **rpc_port** | Your validator `rpc` port that is defined in `config.toml` file. Default value for osmosis is `16657` |
| **grpc_port** | Your validator `grpc` port that is defined in `app.toml` file. Default value for osmosis is `16090` |
...
To add validator use command with specified `VALIDATOR_IP`, `OSMOSIS_VALOPER_ADDRESS`, `OSMOSIS_WALLET_ADDRESS` and `PROJECT_NAME`
```
$HOME/cosmos_node_monitoring/add_validator.sh VALIDATOR_IP OSMOSIS_VALOPER_ADDRESS OSMOSIS_WALLET_ADDRESS PROJECT_NAME
```

> example: ```$HOME/cosmos_node_monitoring/add_validator.sh 1.2.3.4 osmosisvaloper1zyyz4m9ytdf60fn9yaafx7uy7h463n7alv2ete osmosis1zyyz4m9ytdf60fn9yaafx7uy7h463n7a05eshc osmosis```
