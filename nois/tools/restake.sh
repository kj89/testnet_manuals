...
for (( ;; )); do
	...
	noisd tx distribution withdraw-rewards $NOIS_VALOPER_ADDRESS --from=$WALLET --commission --chain-id=$NOIS_CHAIN_ID --gas-prices=0.05unois --yes
	...
	noisd tx staking delegate $NOIS_VALOPER_ADDRESS $AMOUNT_STRING --from $WALLET --chain-id $NOIS_CHAIN_ID --gas-prices=0.05unois --yes
