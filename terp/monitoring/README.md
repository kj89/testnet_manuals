...
# Set up monitoring and alerting for terp validator
...
| KEY |VALUE |
|---------------|-------------|
| **bond_denom** | Denominated token name, for example, `uterpx` for terp testnet. You can find it in genesis file |
| **bench_prefix** | Prefix for chain addresses, for example, `terp` for terp testnet. You can find it in public addresses like this **terp**_valoper1zyyz4m9ytdf60fn9yaafx7uy7h463n7alv2ete_ |
| **rpc_port** | Your validator `rpc` port that is defined in `config.toml` file. Default value for terp is `33657` |
| **grpc_port** | Your validator `grpc` port that is defined in `app.toml` file. Default value for terp is `33090` |
...
To add validator use command with specified `VALIDATOR_IP`, `TERP_VALOPER_ADDRESS`, `TERP_WALLET_ADDRESS` and `PROJECT_NAME`
```
$HOME/cosmos_node_monitoring/add_validator.sh VALIDATOR_IP TERP_VALOPER_ADDRESS TERP_WALLET_ADDRESS PROJECT_NAME
```

> example: ```$HOME/cosmos_node_monitoring/add_validator.sh 1.2.3.4 terpvaloper1zyyz4m9ytdf60fn9yaafx7uy7h463n7alv2ete terp1zyyz4m9ytdf60fn9yaafx7uy7h463n7a05eshc terp```
