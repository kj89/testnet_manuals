...
for (( ;; )); do
	...
	echo -e "${PASSWORD}\n" | Cardchain tx distribution withdraw-rewards $CARDCHAIN_VALOPER_ADDRESS --from=$WALLET --commission --chain-id=$CARDCHAIN_CHAIN_ID --fees=250ubpf --yes
	...
	echo -e "${PASSWORD}\n" |  Cardchain tx staking delegate $CARDCHAIN_VALOPER_ADDRESS $AMOUNT_STRING --from $WALLET --chain-id $CARDCHAIN_CHAIN_ID --fees=250ubpf --yes
